...
export const actions: Actions = {
	addMember: async ({ params, request }) => {
		...
		const { error: e } = await supabase.from('members').insert({ group_id: groupId, name });
	...
	addExpense: async ({ params, request }) => {
		...
		if (!description) return fail(400, { addExpenseError: 'Description is required.' });
		...
		const memberIds = new Set([paidBy, ...participants, ...(createdBy ? [createdBy] : [])]);
		const { data: memberRows, error: mErr } = await supabase
			.from('members')
			.select('id')
			.eq('group_id', groupId)
			.in('id', [...memberIds]);
		if (mErr) return fail(500, { addExpenseError: 'Could not validate members.' });
		const validIds = new Set((memberRows ?? []).map((r) => r.id as string));
		if (!validIds.has(paidBy) || !participants.every((p) => validIds.has(p))) {
			return fail(400, { addExpenseError: 'Members no longer exist. Refresh and try again.' });
		}
		const createdByValid = createdBy && validIds.has(createdBy) ? createdBy : null;
		...
		const { data: expense, error: expErr } = await supabase
			...
			.select('id')
			.single();
		if (expErr || !expense) return fail(500, { addExpenseError: 'Could not add expense.' });
	...
	deleteExpense: async ({ params, request }) => {
		...
		if (!expenseId) return fail(400, { deleteExpenseError: 'Missing expense id.' });
		...
		if (exp.created_by && exp.created_by !== actingAs) {
			return fail(403, { deleteExpenseError: 'Only the person who added this expense can delete it.' });
		}
